...
\begin{document}
	...
	\begin{figure}[!htb]
		...
		\caption{Parameter sweep on the tuner mass $m_t \in [0, 400]$\,g (other parameters fixed). From top to bottom: (i)~$\dot\theta(L,t_b)$ varies little (5.9 to 6.7 MOA/ms) because $t_b/T_1 \approx 0.085$ remains in the initial rising quarter cycle; (ii)~$\theta(L,t_b)$, on the contrary, varies strongly and monotonically ($+166$ to $-255$ µrad), reflecting a shift of the mean point of impact; (iii)~$f_1$ decreases monotonically from 39.9 to 30.3 Hz (relation predicted by the Rayleigh quotient, equation \eqref{eq:dwdmt}).}
		\label{fig:balayage_tuner}
	\end{figure}

	\paragraph{A caveat on this agreement.} The $\sim 10\,\%$ agreement ($+6.64$ vs $6.0$ MOA/ms) must be read with caution, because the simulator's \emph{exponential} internal-ballistics model realises, on its own, a sensitivity $\tau_v = -\partial t_b/\partial v_0 \approx 6.5\ \mu$s/(m/s), not the $8.8\ \mu$s/(m/s) measured by Kolbe and used in the analytical criterion \eqref{eq:dthetaopt}. No setting of the single time constant $\tau$ reconciles both of Kolbe's constraints at once: imposing $\tau_v = 8.8$ requires $t_b \approx 3.3$~ms, whereas imposing $t_b \approx 2.8$~ms gives back $\tau_v \approx 7.4$. It is therefore the \emph{shape} of the profile $v(t)$, not its time scale, that is the present limit of the kinematic coupling --- the natural refinement for later quantitative work. The numerical agreement in $\dot\theta(L,t_b)$ holds here because $h_\text{offset}$ absorbs most of the amplitude discrepancy; it does not amount to a validation of the internal ballistics itself.
