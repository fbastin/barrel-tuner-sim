...
\begin{document}
	...
	Three results emerge. First, \emph{position alone suffices to tune}: there is no need to change weights, as the tuner travel spans the whole useful range. Second, \emph{the weight determines where along the travel}, not whether tuning is possible: the target is reached near $d \approx 90$~mm at 100\,g and near $d \approx 65$~mm at 200\,g --- the lighter the mass, the further the sweet spot from the muzzle. Third, \emph{the optimum is broad}, and the more so the heavier the mass: the region within 1 MOA/ms of the target spans $\sim 30$~mm of travel at 100\,g and $\sim 65$~mm at 200\,g. Mass and position thus form a single \emph{tuning space}, the weight fixing the curve and the position tuning along it.
