...
\begin{document}
	...
	\begin{enumerate}
		...
		\end{equation}
		Numerically, at $D=50$\,m and $v_0 \approx 308$\,m/s (1010 ft/s, .22 LR Match), $\partial h_\text{bal}/\partial v_0 \approx -0.84$\,mm/(m/s), i.e.\ roughly $-0.016$ MOA per ft/s; this is also the number computed by Kolbe~\cite{Kolbe} using a trajectory solver.
	...
	\end{equation}
	\emph{The negative signs of $\partial h_\text{bal}/\partial v_0$ and $\partial t_b/\partial v_0$ combine to make $\dot\theta_\text{out}^\star$ positive: the muzzle must therefore be rising ($\dot\theta>0$) at the moment of shot release.} Numerically, with $D=50$\,m, $v_0 = 308$\,m/s and $\tau_v = 8.8\,\mu$s/(m/s):
	\begin{equation*}
		\dot\theta_\text{out}^\star \;\approx\; 1.9\ \text{mrad/ms} \;\approx\; 6.6\ \text{MOA/ms},
	\end{equation*}
	to be compared with the $6.0$ MOA/ms experimentally measured by Kolbe (the $\sim 10\,\%$ discrepancy is explained by the non-flatness of the trajectory at 50 m and the approximations on $\tau_v$).
