...
\begin{document}
	...
	\begin{figure}[!htb]
		...
		\caption{Tuning by position at fixed mass, for two masses bracketing the real range (100 and 200\,g). Top: $\dot\theta(L,t_b)$ against the overhang $d$; the green band marks $\pm 1$ MOA/ms around Kolbe's target. The overhang alone moves the angular rate from $+0.8$ to $+6.2$ MOA/ms (100\,g) or from $+1.8$ to $+5.9$ (200\,g): in both cases the travel spans the entire gap between an uncompensated rifle and the target. Bottom: $f_1$ decreases with $d$, position shifting the same modal frequencies as mass does.}
		\label{fig:balayage_position}
	\end{figure}

	Three results emerge. First, \emph{position alone suffices to tune}: there is no need to change weights, as the tuner travel spans the whole useful range. Second, \emph{the weight determines where along the travel}, not whether tuning is possible: the target is reached near $d \approx 80$~mm at 100\,g and near $d \approx 65$~mm at 200\,g --- the lighter the mass, the further the sweet spot from the muzzle. Third, \emph{the optimum is broad}, and the more so the heavier the mass: the region within 1 MOA/ms of the target spans $\sim 45$~mm of travel at 100\,g and $\sim 65$~mm at 200\,g. Mass and position thus form a single \emph{tuning space}, the weight fixing the curve and the position tuning along it.
