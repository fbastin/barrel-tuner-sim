...
\begin{document}
	...
	As an order of magnitude, the drop is $h = \tfrac12\,g\,(D/\bar v)^2$ for a distance $D$ and a muzzle velocity $\bar v$, so that a velocity spread $\Delta v$ shifts the impact by $|\Delta h| \approx g\,D^2\,\Delta v / \bar v^3$. For a match .22~LR ($\bar v \approx 330$~m/s) fired at $D = 50$~m, a spread of $30$~fps ($\Delta v \approx 9$~m/s) therefore produces, through time of flight alone, a vertical streak of about $6$~mm --- easily enough to ruin a match group. It is precisely this spread that positive compensation sets out to cancel.

	\paragraph{What this bounds, and why the gain is counter-intuitive.} This term is the \emph{only} one a tuned tuner removes: the ammunition's own dispersion, the wind and the shooter are left untouched. Assuming these sources independent, they combine in quadrature, so that a group $G$ can at best shrink to
	\begin{equation}
		G_\text{res} \;=\; \sqrt{G^2 - \Delta h^2}, \qquad \Delta h \approx g\,D^2\,\Delta v / \bar v^3.
		\label{eq:gres}
	\end{equation}
	This bound involves \emph{no} vibratory quantity --- neither amplitude, nor frequency, nor tuner mass --- and is therefore computed from three figures the shooter already measures: distance, chronograph velocity spread and habitual group size. It is, on that count, the most robust prediction in this document, whereas the absolute values of $\dot\theta$ in section~\ref{sec:validation_num} and beyond remain calibrated orders of magnitude. Its consequence is counter-intuitive: \emph{the better the shooter, the more a tuner returns}. In .22~LR at 50~m with $\Delta v = 10$~m/s, velocity spread accounts for $7.6$~mm; on a $20$~mm group, removing it gains only $8\,\%$, whereas on an $8$~mm group the gain reaches $70\,\%$. The tuner is thus structurally a \emph{benchrest} tool, which also explains why reports diverge so widely from one shooter to the next. This bound holds only for recoiling firearms, however (cf.\ section~\ref{sec:pcp}).
	...
	The \texttt{Julia} simulator accompanying this document (\texttt{simulation.jl}) implements the full formalism: FEM assembly, clamping, tuner, internal ballistics (``burnout'' profile: acceleration over a fraction $\phi=0.35$ of the barrel, then coast, calibrated on Kolbe's $\tau_v$), excitation by the breech recoil moment $M(t) = p(t)\,A_\text{bore}\,h_\text{offset}$, moving projectile load, Rayleigh damping and Newmark-$\beta$ scheme. There the lever arm $h_\text{offset}$ is $16.5$~mm. It is an \emph{effective} quantity, not a dimension: it places the model at the compensation optimum for a tuner setting lying within the real adjustment range. Its physical referent comes from the only published measurement of the excitation itself, Vaughn's~\cite{Vaughn} (strain gauges on the forward receiver ring of a .270~Win, chapter~4): the \emph{applied} moment he reports, $\sim 1500$~in-lb, corresponds to a lever arm of $11.9$~mm, hence $\sim 9$ to $12$~mm once carried over to .22~LR through \eqref{eq:hoffset_phys}. The ratio, of order $1.4$ to $1.8$, \emph{quantifies} what the clamped frame omits --- the rigid-body rotation of the firearm under recoil. Adopting the physical value instead, in the amputated model, makes it unusable: it then saturates at $3.5$ MOA/ms and never reaches the $6.0$ optimum, in mass or in position. The only quantity confronted with measurement remains $\dot\theta(L,t_b)$.
	...
	\subsection{Tuning by position: the effective adjustment parameter}
	\label{sec:position}

	The sweep in figure~\ref{fig:balayage_tuner} varies the \emph{mass} $m_t$, a convenient parameter in the model. It is not, however, the adjustment variable used in the field: there the mass is \emph{fixed} (a given weight is fitted) and tuning is done by \emph{screwing} the tuner further out or in, overhanging ahead of the muzzle --- a continuous adjustment counted in thread turns.

	This degree of freedom is modelled by offsetting the tuner mass a distance $d$ ahead of the muzzle node. The mass--rotation coupling of an offset mass adds to the degree-of-freedom pair $(y_L,\theta_L)$ the contribution
	\begin{equation}
		\mathbf{M}_\text{add} =
		\begin{bmatrix}
			m_t & m_t d \\[2pt]
	...
	\end{equation}
	where $J_\text{cm}$ is the tuner's own inertia about its centre of mass; at $d = 0$ the point mass of the previous model is recovered.

	\begin{figure}[!htb]
		\centering
		\includegraphics[width=0.85\linewidth]{plot_balayage_position.png}
		\caption{Tuning by position at fixed mass, for two masses bracketing the real range (100 and 200\,g). Top: $\dot\theta(L,t_b)$ against the overhang $d$; the green band marks $\pm 1$ MOA/ms around Kolbe's target. The overhang alone moves the angular rate from $-1.7$ to $+6.2$ MOA/ms (100\,g) or from $+1.8$ to $+5.9$ (200\,g): in both cases the travel spans the entire gap between an uncompensated rifle and the target. Bottom: $f_1$ decreases with $d$, position shifting the same modal frequencies as mass does.}
		\label{fig:balayage_position}
	\end{figure}

	Three results emerge. First, \emph{position alone suffices to tune}: there is no need to change weights, as the tuner travel spans the whole useful range. Second, \emph{the weight determines where along the travel}, not whether tuning is possible: the target is reached near $d \approx 100$~mm at 100\,g and near $d \approx 65$~mm at 200\,g --- the lighter the mass, the further the sweet spot from the muzzle. Third, \emph{the optimum is broad} in both cases: the region within 1 MOA/ms of the target spans $\sim 95$~mm of travel at 100\,g and $\sim 75$~mm at 200\,g. Mass and position thus form a single \emph{tuning space}, the weight fixing the curve and the position tuning along it.

	\paragraph{Typical masses.} Manufacturers' published weights bracket the range:
	\begin{center}
	\begin{tabular}{lcc}
		\toprule
		Tuner & Published weight & Intended use \\
		\midrule
		PMA 7/8-32 & 4.2 oz (119 g) & centrefire \\
		Ezell PDT \emph{light} & 5 oz (142 g) & centrefire \\
		Ezell PDT standard & 7 oz (198 g) & centrefire \\
		Harrell's Rimfire/Air & 8 oz (227 g) & rimfire \\
		Starik/Centra carbon tube & $\sim$200 to 220 g (complete assembly) & rimfire ISSF \\
		\bottomrule
	\end{tabular}
	\end{center}
	that is, $\sim 115$ to 230~g for a muzzle tuner. These figures do not, however, describe comparable objects: with Harrell's or Ezell the entire body constitutes the mass, tuned by screwing it over a few millimetres of thread; the Starik carbon tube is the opposite architecture, a light tube in which a small mass slides over several centimetres, so that its $\sim 200$~g assembly weight is not the moving mass, which is not published. The two curves in figure~\ref{fig:balayage_position} bracket both regimes.

	\paragraph{Aim for the node, not the number.} Section~\ref{sec:cinematique} states the compensation condition in two equivalent forms: an angular rate of $6$ MOA/ms, but also and above all an \emph{exit near a rising temporal node}, where $\theta = 0$ and $\dot\theta$ is maximal. The two forms coincide by quadrature. Tuning on the \emph{number}, however, does not always lead there, and the sweep above supplies a counter-example. As long as the excitation was assumed identical on every shot, the discrepancy had no visible consequence; once it varies (point~12) it is paid for, the sensitivity to that variability being proportional to the \emph{absolute} angle $\theta(t_b)$. At 200\,g the model saturates at $5.91$ MOA/ms and never reaches $6.0$: ``closest to target'' therefore falls back on the maximum, i.e.\ on the node, and the $65$~mm setting is already the right one. At 100\,g, by contrast, $\dot\theta$ crosses $6.0$ around $100$~mm and peaks at $6.19$ near $115$--$120$~mm: ``closest to $6.0$'' therefore selects $100$~mm, \emph{short} of the maximum, $120$~$\mu$rad away from neutral instead of $10$. The variability simulator (\texttt{variability.jl}) puts the gap at a \textbf{factor of 2.8} in predicted dispersion ($0.65$ against $0.23$~mm standard deviation at 50~m), the optimum moving to $\sim 120$~mm (a bracket, not a dimension: the node's position depends on the tuner's own inertia, hence on its architecture --- the model describes a thin-walled tube assembly here, whose radius of gyration follows from the mass installed, about $2.8$~cm at 100\,g and $5.0$~cm at 200\,g; the dispersion reached at the node stays $\sim 0.23$~mm whatever that inertia). That dimension has in fact already moved: an earlier version quoted $110$~mm, the tuner's inertia being treated there as a constant independent of the mass installed --- the correction shifts the dimension but leaves the dispersion at the node unchanged at $0.23$~mm, which is precisely the reservation stated above. Such an overhang is only achievable with the tube architecture (Starik/Centra, tubes of 19 to 36~cm), not with a screw-on body; the added static droop is negligible (a few hundredths of a millimetre) and the connection stays rigid, a 100\,g mass on some ten centimetres of tube resonating near 600~Hz, far above the fundamental mode. The $100$~mm quoted above is therefore a \emph{methodological artefact}, not an optimum. The result also vindicates the \emph{ladder tune}: a shooter who sweeps the travel and keeps the best group finds the node without computing it, where aiming at an angular-rate figure may lead away from it. \emph{Status}: the mis-placement follows from the criterion of section~\ref{sec:cinematique} and is readable off the sweep; its quantified \emph{cost}, however, depends on the variability model and its least secure parameter --- a testable, unvalidated prediction.

	\paragraph{Fineness of adjustment.} The steps recorded on a Starik/Centra tuner --- weight carrier in 10~mm increments, fine adjustment at 5 turns per 2.5~mm, i.e.\ a 0.5~mm pitch --- correspond, on the curve of figure~\ref{fig:balayage_position}, to $\sim 0.01$ MOA/ms per turn near the optimum (up to $\sim 0.04$ on the steep part). The width of the optimum, $\sim 150$ to $190$ turns, explains why a \emph{ladder tune} converges without demanding turn-level precision.
	...
	\paragraph{11. An unsuccessful recreation of Harral's study.} The analysis of point~9 was replayed within the FEM framework of this document --- an Euler--Bernoulli beam of \emph{variable section} reproducing Esten's \emph{reverse taper} contour, Newmark-$\beta$ transient response, then Harral's point-of-impact formula (muzzle projection $+$ vertical velocity $\times$ time of flight $-$ drop). The recreation \emph{fails}, and the diagnosis illuminates the limits of the clamped model. Three causes were identified, two of them corrected.

	\emph{(i) Harral's muzzle projection is not vibration but static sag} (corrected). His page states it explicitly: ``Gravity is applied and the stock deforms as well as the barrel sag.'' His $-1.32''\to-2.42''$ describe a barrel sagging under the increasing weight of the tuner, before the shot is even fired. The script initially applied no gravity at all; it now applies the self-weight of barrel and tuner and starts from static equilibrium $\mathbf{U}_0 = \mathbf{K}_a^{-1}\mathbf{F}_\text{grav}$, giving $-1.13 / -1.41 / -1.61 / -2.02''$ against Harral's $-1.32 / -1.72 / -1.96 / -2.42''$: the same trend and the same order of magnitude \emph{with no fitting whatsoever}, the residual gap corresponding to the stock, which Harral models and this script ignores.

	\emph{(ii) The calibration compared a dynamic amplitude with a static number} (corrected). Scaling the peak of the oscillation to $\approx 1.5''$ --- that is, to Harral's \emph{static} sag --- inflated the amplitude by roughly an order of magnitude. With the static sag now produced by gravity, that scaling has gone; but the consequence must be drawn: \emph{nothing in Harral's published data constrains the dynamic amplitude any longer}. It has become an acknowledged free parameter.

	\emph{(iii) The boundary conditions describe a different system} (the decisive cause, unresolved). Harral models the entire rifle, free to recoil and rotate: ``The stock contacts the simulated sandbag rests with zero friction in the calculation. The rests are fixed in space.'' This is a \emph{frictionless unilateral support} --- neither a clamp nor a free-floating rifle. The clamped model, by contrast, suppresses all whole-body rotation, precisely the mechanism from which positive compensation arises (point~10). Only elastic bending remains: the 35~Hz fundamental is out of play and the muzzle angle oscillates at $\sim 400$~Hz, so that $t_b$ falls on a steep slope of this rapid oscillation. The value read there is not a compensation but a sample taken at an arbitrary instant, and the exit cannot be shifted to recover the phase: at 315~m/s over 63~cm, the absolute floor is 2.0~ms.

	Adjusting the amplitude corrects nothing, as the addition of gravity demonstrated. Before correction, the computed dispersion \emph{decreased} as the muzzle was loaded ($0.148''$ bare $\to 0.085''$ at 16~oz) where Harral reports it \emph{increasing} ($0.091'' \to 0.183''$): the opposite physical conclusion. Once the amplitude was brought back to a realistic value and the internal ballistics corrected (burnout profile, $t_b = 2.69$~ms), the inversion disappears and the dispersion clusters around the \emph{uncompensated} value --- $0.159 / 0.168 / 0.170 / 0.169''$ for the four masses, all close to $0.173''$, with no trace of Harral's structure. The system being linear, this was predictable: what is missing is not a scale factor but the \emph{rigid-body rotation} of the rifle under recoil. It is therefore a modelling gap, not an implementation error --- a gap since closed by the free-rifle model (section~\ref{sec:validation_num}).

	\emph{Methodological caveats.} The ``rigid muzzle'' line ($0.173''$) recovered to the hundredth is not a validation: it is obtained by subtracting two drop values Harral publishes himself, with no simulation involved. Nor can his modal analysis be reproduced, since \emph{he publishes no natural frequencies}. Finally, a trap awaits anyone replaying the exercise: in Harral's work, table and charts do not share the same reference --- ``The sag due to gravity was in the calculations, but subtracted out for the chart.'' The static sag is removed from the \emph{curves}, not from the \emph{proj} column of the table.

	\paragraph{12. An excitation assumed identical on every shot.} The model applies \emph{exactly} the same recoil moment shot after shot: one and the same cartridge produces one and the same vibration, hence one and the same muzzle angle at exit. The assumption is convenient --- it licenses speaking of \emph{one} exit instant and \emph{one} optimal setting --- but it is false, and the discrepancy has been measured. Vaughn~\cite{Vaughn} instrumented the receiver ring with strain gauges: on a .270~Win, the peak moment is nominally 450~in-lb but \emph{varies from 300 to 600~in-lb between shots with the same lot}, i.e. $\sim \pm 30\,\%$, the timing of the peak shifting as well. He attributes most of it to bolt-thrust variations and receiver asymmetries. On his unmodified rifle this \emph{variability} --- not the mean vibration --- accounts on its own for $\sim 0.8$~inch ($\sim 20$~mm) of dispersion at 100~yards. Three consequences for this document: a tuner set on \emph{mean} behaviour still suffers that spread, which is a \emph{floor} on a par with the ammunition's own dispersion; the narrow optimality window of section~\ref{sec:cinematique} is in fact \emph{blurred}, since $t_b$ and the amplitude do not repeat exactly; and the recoil moment is not the only source --- Vaughn identifies \emph{three} (recoil on the lug, bolt thrust on unevenly engaged lugs, receiver asymmetry) where \eqref{eq:moment_recul} retains one, and removing the recoil component alone \emph{does not} null the moment but exposes an opposite one. These figures are for a centerfire hunting rifle: their magnitude does not carry over to the match .22~LR, but the mechanism does.

	\paragraph{Summary of epistemic status.} The material gathered in this document is of unequal nature and benefits from being distinguished: \emph{measured} --- the principle of positive compensation and the 6.0 MOA/ms criterion (Kolbe, two concordant independent methods); \emph{computed but unvalidated} --- Harral's study, from which the warning of point~9 derives; \emph{failed} --- the recreation above, for want of whole-body rotation; \emph{outside the model} --- horizontal vibration (point~7), rimfire excitation beyond the recoil moment alone (point~6), and the shot-to-shot variability of that excitation, measured at $\pm 30\,\%$ (point~12).

	\section{The pre-charged pneumatic (PCP) case}
	\label{sec:pcp}

	Manufacturers of high-end air rifles offer muzzle \emph{harmonic tuners}. These devices do have an effect, but through a mechanism distinct from the one developed in this document, and the distinction deserves to be stated precisely, as two schools of tuning coexist.

	\paragraph{Positive compensation is largely out of play.} The whole formalism above rests on \emph{recoil}: it is recoil that rotates the rifle about its centre of gravity and imparts the driving moment \eqref{eq:moment_recul}. A PCP, however, is nearly recoilless, some models being designed as such. The dominant excitation term therefore collapses, and nothing guarantees any longer that the muzzle rises at the rate required by \eqref{eq:dthetaopt}: that rate derived precisely from the recoil rotation.

	\paragraph{The operative mechanism is the ``stationary muzzle''.}
	\begin{center}
	\begin{tabular}{lccc}
		\toprule
		School & Condition at exit & Effect & Requires recoil \\
		\midrule
		Positive compensation & $\dot\theta$ maximal, positive & masks velocity spread; & yes \\
		(Kolbe, this document) & (muzzle rising) & distance-specific & \\
		Stationary muzzle & $\dot\theta \approx 0$ & makes the launch angle insensitive & no \\
		(node) & (turning point) & to jitter in $t_b$ & \\
		\bottomrule
	\end{tabular}
	\end{center}

	The second condition does not depend on the excitation source. A PCP barrel does vibrate despite the absence of recoil, through the hammer strike on the valve, the air blast and the pellet's passage (friction, engraving, moving-load effect) --- this last term being already present in the model. A muzzle weight shifts the modal frequencies and moves $t_b$ within this whip cycle.

	At an extremum of $\theta$, $\dot\theta$ vanishes but $\ddot\theta$ does not: the residual dispersion there is \emph{second order} in $\Delta t$, varying as $\tfrac{1}{2}\ddot\theta\,\Delta t^2$. It is therefore minimised, not cancelled --- the fundamental difference from positive compensation, which \emph{uses} the first-order term $\dot\theta\,\Delta t$ to cancel the ballistic drop where the stationary muzzle \emph{eliminates} it. Two confusions must be avoided: aiming at a maximum of $\dot\theta$ (the positive-compensation target, which requires recoil) instead of $\dot\theta \approx 0$; and aiming at an extremum of the \emph{displacement} $y(t)$ rather than of the \emph{angle} $\theta(t)$, when Kolbe measured that the muzzle's translational velocity contributes negligibly to dispersion. In the real transient, $\theta$ and $y$ peak at different instants.

	\paragraph{Structure of a simulator.} Counter-intuitively, the PCP moves \emph{away} from Harral's ``free rifle'' scheme. That elaborate scheme (free recoil, unilateral supports, rigid-body degrees of freedom) serves one purpose: to capture the whole-rifle rotation driven by recoil. As this term is negligible on a PCP, the shortcoming of the clamped model becomes immaterial: the clamped framework is in fact closer to its own assumptions for a PCP than for the .22~LR for which it was written. The bulk of the work therefore concerns the \emph{excitation}: remove the recoil moment, add a hammer/valve impulse together with a realistic moving load, and target $\dot\theta(L,t_b) \approx 0$.

	\paragraph{Results.} This structure is implemented in \texttt{pcp\_tuner.jl}. It exploits a property that makes the computation possible despite the unknown excitation: the system being linear, the \emph{location} of the sweet spots ($\dot\theta = 0$) depends only on the unit response --- mode shapes and exit time --- not on the excitation amplitude. One can therefore predict \emph{at which setting} the muzzle becomes stationary without measuring the hammer strike; only the \emph{depth} of the gain remains out of reach.

	The computation corrects a common assumption: one readily pictures a ``comb'' of closely spaced sweet spots. The whip does oscillate rapidly in time ($\sim 250$--700~Hz), but its frequency shifts by only $\sim 20\,\%$ when the tuner is loaded with 100~g, so the phase at $t_b$ sweeps only a fraction of a cycle. On a representative .22 airgun barrel, $\dot\theta(t_b)$ crosses zero only once within the realistic range. As in .22~LR (section~\ref{sec:position}), the effective adjustment is \emph{position}: at fixed mass, $\dot\theta(t_b)$ crosses zero at a definite overhang --- near 63~mm for 70~g, with a broad optimum ($\sim 18$~mm). The mass shifts this sweet spot along the travel following a regular tuning curve (89~mm at 40~g, 63~mm at 70~g, 44~mm at 100~g): the heavier the weight, the closer the sweet spot to the muzzle.

	\begin{figure}[!htb]
		\centering
		\includegraphics[width=0.85\linewidth]{plot_pcp_courbe_accord.png}
		\caption{Tuning curve of a .22 PCP tuner: overhang of the sweet spot (stationary muzzle, $\dot\theta = 0$) as a function of the fixed mass. These values are specific to the simulated barrel; it is the \emph{shape} of the relation that is robust, not the millimetre figures.}
		\label{fig:pcp_accord}
	\end{figure}

	\paragraph{Reservations.} The dominant accuracy variable on a PCP lies elsewhere: regulator consistency and pellet/velocity matching. A tuner compensates neither for a poorly set regulator nor for bad pellets, and on a well-regulated PCP little velocity spread remains to be masked. Many gains attributed to the tuner moreover stem from a pellet change tested at the same time --- the same methodological pitfall as in .22~LR. Finally, the depth of the benefit depends on the excitation amplitude, which is not measured: the model gives the \emph{where}, not the \emph{how much}.
	...
	\begin{thebibliography}{9}
		...
		\bibitem{Vaughn}
		H. R. Vaughn, \textit{Rifle Accuracy Facts}, Precision Shooting Inc., 1998. --- The only source measuring the \emph{excitation} itself (receiver-ring strain gauges, muzzle accelerometer), in centerfire.
